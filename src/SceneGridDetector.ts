...
// Base level id stamped by the DDB importer (matches DDBMap.ts /
// NativeSceneBuilder). Defined locally to avoid a muncher import (cycle risk).
...
const CANDIDATE_LABELS: Record<CandidateKey, string> = {
  ...
  priorPeriod: "Stored token scale + detected offset",
  tokenScale: "Stored token scale",
  tokenScaleDoubled: "Stored token scale x 2",
  tokenScaleHalved: "Stored token scale / 2",
};
...
export interface ISceneGridFlagOptions {
  flagScope?: string;
  fallbackFlagScopes?: string[];
  minGridSize?: number;
}

function normaliseFlagOptions(options: ISceneGridFlagOptions = {}) {
  const flagScope = options.flagScope ?? PRIMARY_FLAG_SCOPE;
  const fallbackFlagScopes = [
    ...(options.fallbackFlagScopes ?? LEGACY_FLAG_SCOPES),
    PRIMARY_FLAG_SCOPE,
  ].filter((scope, index, all) => scope !== flagScope && all.indexOf(scope) === index);
...
export interface IRunDetectionOptions {
  ...
  // the 5-ft-cell prior - covers the case where a scene's tokenScale
  ...
  // Scene flag scope used for tokenScale reads and grid result writes.
  flagScope?: string;
  fallbackFlagScopes?: string[];
  minGridSize?: number;
...
async function runDetectionOnRoi(
  ...
  // over the stored prior. cellPx is already the painted period in image
...
export async function runDetectionForScene(
  ...
  const detectorOpts = {
    ...
    flagScope: options.flagScope,
    fallbackFlagScopes: options.fallbackFlagScopes,
  };
  ...
  const grid = resolveGrid({
    ...
    multiplier,
    minGridSize: options.minGridSize,
  });
...
export async function applyChoiceToScene(
  ...
  // image fills the base, and write the document grid offset. When a base level
  // is present, also reset its texture to identity to undo any scale a prior run
  // left behind (which is what caused the image to spill into the padding).
  const updatePayload: any = {
    width: sceneWidth,
    height: sceneHeight,
    grid: gridPayload,
    flags: { [flagScope]: { ...sharedFlags, gridLevelId: levelId ?? null } },
  };
