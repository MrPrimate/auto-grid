...
const AutoGridApplication = HandlebarsApplicationMixin(ApplicationV2);
...
export default class SceneGridPickerApp extends AutoGridApplication {
  ...
  // Foundry cells per detected painted square. Defaults to AutoGrid's
  // configured multiplier; user can pick 1, 2, 3, or 4 in the dropdown for
  // per-detection control. For imported scenes we instead
  ...
  static DEFAULT_OPTIONS = {
    id: "auto-grid-picker",
    classes: ["dnd5e2", "auto-grid-picker-app"],
    window: {
      title: "Detect Grid",
  ...
  static PARTS = {
    content: {
      template: "modules/auto-grid/templates/scene-grid-picker.hbs",
    },
  };
  ...
  // Tries three signals in priority order:
    //   1. Explicit `gridMultiplier` flag (written by AutoGrid and integrations
  ...
  //      multiplier). Covers legacy scenes set up before the flag.
    //   3. AutoGrid's default multiplier setting.
  ...
  _inferMultiplierFromScene() {
    ...
    const flagScopes = [
      this.workflowOptions.flagScope ?? "auto-grid",
      ...(this.workflowOptions.fallbackFlagScopes ?? ["ddbimporter"]),
    ].filter((scope, index, all) => all.indexOf(scope) === index);
    const flags = flagScopes
      .map((scope) => this.scene.flags?.[scope] ?? {})
      .find((scopeFlags) => Object.keys(scopeFlags).length > 0) ?? {};
    const flagged = flagScopes
      .map((scope) => this.scene.flags?.[scope]?.gridMultiplier)
    ...
    const storedDetection = flagScopes
      .map((scope) => this.scene.flags?.[scope]?.gridDetection)
      .find((value) => value !== undefined);
  ...
  // scenes from older integrations where gridMultiplier was not stamped
  ...
  _inferMultiplierFromDetection() {
    ...
    // (tokenScale * imageWidth - the stored 5-ft cell hint) is often cleaner than
    ...
    try {
      ...
      if (this._multiplierUserSet) {
        ...
        const passes = await Promise.all([1, 2, 3, 4].map(async (m) => {
          const opts = { ...baseOptions, ...this.workflowOptions, multiplier: m, searchPaddingFraction: 0.4 };
  ...
  static async applyChoice(this: SceneGridPickerApp) {
    ...
    try {
      await applyChoiceToScene(this.scene, this.run, choice, this.workflowOptions);
